...
this.GetComponent<Renderer>().sharedMaterial.SetFloat("_Distortion", SlimDistortion);
...
this.GetComponent<Renderer>().sharedMaterial.SetFloat("TurnToLiquid", TurnToSlim);
}
else if(this.gameObject.GetComponent<Image>() != null)
{
...
#if UNITY_EDITOR
[CustomEditor(typeof(_2dxFX_AL_Slim)),CanEditMultipleObjects]
public class _2dxFX_AL_Slim_Editor : Editor
{
...
_2dxFX_AL_Slim _2dxScript = (_2dxFX_AL_Slim)target;
...
EditorGUILayout.PropertyField(m_object.FindProperty("TurnToSlim"), new GUIContent("Turn To Slime", icone, "Turn to slime = 0 = normal sprite : 1 = Slime FX"));
icone = Resources.Load ("2dxfx-icon-time") as Texture2D;
EditorGUILayout.PropertyField(m_object.FindProperty("SlimDistortion"), new GUIContent("Slime Distortion", icone, "Change the distortion of the Slime"));
