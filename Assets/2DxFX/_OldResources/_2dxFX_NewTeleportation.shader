...
float4 rtxt = txt2.r;
rtxt.a = talpha;
float3 blue = float3(0.2+txt2r.r*0.3, 0.3 + txt2r.r*0.5, 0.6 + txt2r.r)*TeleportationColor.rgb;
float step1 = smoothstep(  0, 0.2, _Fade);
float hdr = lerp(1, _HDR_Intensity, step1);
rtxt.rgb = lerp(txt1.rgb, txt1.rgb*.25+blue, step1);
...
float4 txt3 = tex2D(_MainTex2, uv - float2(txt2.r*0.05*_Distortion, _Fade*2))*8;
btwa += txt3.g*2;
